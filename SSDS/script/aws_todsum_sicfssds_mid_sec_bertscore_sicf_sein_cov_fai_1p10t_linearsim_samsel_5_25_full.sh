...
if [ ${train_percent} == 1 ];
then
  ori_gt_labtr_file="data/${datasetname}/labeled_train_2per.json" # divided the original: 1% of original used as labeled data
elif [ ${train_percent} == 5 ];
then
  ori_gt_labtr_file="data/${datasetname}/labeled_train_10per.json" # divided the original: 5% of original used as labeled data
fi
...
unc_save_prefix="newTODSUM_1p10t_full_bnn_mean_dp_select_linearrank_aws" # using new cov by POS
...
python SSDS_main.py \
    --model_name_or_path ${ori_ini_model_name_or_path} \
    --do_train False \
    --do_eval False \
    --do_predict False \
    ...
    --search_sample_mode bnn_mean_dp \
